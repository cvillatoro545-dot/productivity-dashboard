...
import { Home as HomeIcon, CalendarDays, Target, Star, BookOpen, PenLine, Moon, Sun, LayoutGrid } from "lucide-react";
...
type Page = "home" | "week" | "quarter" | "year" | "areas" | "knowledge" | "journal" | "tasks" | "habits" | "notes" | "books" | "bucket";

const NAV_ITEMS = [
  { id: "home" as Page,      label: "Home",      icon: HomeIcon,     },
  { id: "week" as Page,      label: "Week",      icon: CalendarDays, },
  { id: "quarter" as Page,   label: "Quarter",   icon: Target,       },
  { id: "year" as Page,      label: "Year",      icon: Star,         },
  { id: "areas" as Page,     label: "Areas",     icon: LayoutGrid,   },
  { id: "knowledge" as Page, label: "Knowledge", icon: BookOpen,     },
  { id: "journal" as Page,   label: "Journal",   icon: PenLine,      },
];
...
export default function DashboardPage() {
  ...
  const isKnowledge = ["knowledge","books","notes","bucket"].includes(page);
  const pageLabel = NAV_ITEMS.find((n) => n.id === page)?.label || "Knowledge";
    ...
    <div className="min-h-screen flex" style={{ background: "var(--bg)" }}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-56 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:relative lg:flex ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
        ...
        <div className="p-6 pb-4">
          ...
          <p className="text-[10px] font-mono tracking-widest uppercase opacity-30 mt-0.5" style={{ color: "var(--ink)" }}>Personal OS</p>
        </div>

        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
          {NAV_ITEMS.map((item) => {
            const active = page === item.id || (item.id === "knowledge" && isKnowledge);
            return (
          ...
          <div className="pt-3 pb-1">
            <div className="h-px opacity-10" style={{ background: "var(--ink)" }} />
          </div>

          {/* Quick access */}
          {[
            { id: "tasks" as Page, label: "Tasks", badge: stats ? String(stats.tasksTodo + stats.tasksInProgress) : null },
            { id: "habits" as Page, label: "Habits", badge: stats ? `${stats.habitsCompletedToday}/${stats.habitsTotal}` : null },
          ].map((item) => (
            <button key={item.id} onClick={() => { setPage(item.id); setSidebarOpen(false); }}
              className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm transition-all"
              style={{ background: page === item.id ? "var(--bg-muted)" : "transparent", color: "var(--ink)", opacity: page === item.id ? 1 : 0.4 }}>
              <span>{item.label}</span>
              {item.badge && <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-full" style={{ background: "var(--bg-muted)", color: "var(--ink)" }}>{item.badge}</span>}
            </button>
          ))}
        </nav>
        ...
        <div className="p-4 space-y-3 border-t" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="flex items-center justify-between">
            <div className="text-xs font-mono opacity-30" style={{ color: "var(--ink)" }}>{format(today, "MMM d, yyyy")}</div>
            <button onClick={toggleDark} className="p-1.5 rounded-lg transition-all hover:opacity-70 opacity-40" style={{ color: "var(--ink)" }}>
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b sticky top-0 z-10"
          ...
          <button onClick={() => setSidebarOpen(true)} className="p-1.5 rounded-lg text-lg" style={{ color: "var(--ink)" }}>☰</button>
          <div className="flex items-center gap-2">
            <span className="text-amber-accent">✦</span>
            <span className="font-display" style={{ color: "var(--ink)" }}>{pageLabel}</span>
          </div>
        ...
        <div className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-8 py-8 pb-24">
          ...
          {page === "home"     && <HomeDashboard tasks={tasks} habits={habits} streak={streak} onTaskToggle={handleTaskToggle} onHabitToggle={async (id) => { await fetch(`/api/habits/${id}`, { method: "POST" }); fetchAll(); }} onNavigate={(tab) => setPage(tab as Page)} onUpdate={fetchAll} />}
          ...
          {(page === "knowledge" || page === "books") && <BooksPanel />}
